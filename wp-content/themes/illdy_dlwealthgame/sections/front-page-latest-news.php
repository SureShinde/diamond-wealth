...
if ( $post_query->have_posts() || $general_title != '' || $general_entry != '' || $button_text != '' ) {
	...
	<section id="latest-news" class="front-page-section">
						...
						<?php while ( $post_query->have_posts() ): ?>
							...
							<div class="illdy-blog-post col-md-4 col-sm-6 col-xs-12">
								<div class="post" style="<?php if ( ! $post_thumbnail ): echo 'padding-top: 40px;'; endif; ?>">
									...
									<a href="<?php the_permalink(); ?>" title="<?php _e( 'Read more', 'illdy' ); ?>" class="post-button"><?php _e( 'Read more', 'illdy' ); ?>
									</a>
								</div><!--/.post-->
							</div><!--/.col-sm-4-->
							<?php $counter ++; ?>
        ...
        <?php if ( $button_text ): ?>
			<a href="<?php echo esc_url( $button_url ); ?>" title="<?php echo esc_attr( $button_text ); ?>" class="latest-news-button"><?php echo esc_html( $button_text ); ?>
			</a>
		<?php endif; ?>
