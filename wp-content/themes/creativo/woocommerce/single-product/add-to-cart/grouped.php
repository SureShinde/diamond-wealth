			...
			$grouped_product_columns = apply_filters( 'woocommerce_grouped_product_columns', array(
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<td class="product_image_small">
							<?php if(has_post_thumbnail()) { ?>
								<?php echo $product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink(), $product_id ) ) . '">' . woocommerce_get_product_thumbnail('shop_thumbnail') . '</a>' : woocommerce_get_product_thumbnail('shop_thumbnail'); ?>
							<?php } 
							 else {
