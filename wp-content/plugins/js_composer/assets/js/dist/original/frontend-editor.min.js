...
if (function() {
    ...
    }(window.jQuery), !window.vc) var vc = {};
! function($) {
...
if (vc.memoizeWrapper = function(func, resolver) {
    ...
    }(window.jQuery, window._), _.isUndefined(window.vc)) var vc = {};
if (window.vc.filters = {
        templates: []
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.ShortcodesBuilder = function(models) {
            ...
            save: function(status) {
                var string = this.getContent(),
                    ...
                    data = {};
                $.each(post_data, function(i, pair) {
                    var name, value;
                    "[]" === pair.name.slice(-2) ? (name = pair.name.replace("[]", ""), value = pair.value, _.isUndefined(data[name]) || data[name] == value ? data[name] = value : (_.isArray(data[name]) || (data[name] = [data[name]]), data[name].push(value))) : data[pair.name] = pair.value
                }), data.vc_post_custom_css = vc.$custom_css.val(), data.content = this.wpautop(string), status && (data.post_status = status, $(".vc_button_save_draft").hide(100) && $("#vc_button-update").text(window.i18nLocale.update_all)), vc.update_title && (data.post_title = this.getTitle()), this.ajax(data, "post.php").done(function() {
                    vc.unsetDataChanged(), vc.showMessage("Successfully updated!")
                })
            },
            ...
            wpautop: function(string) {
                var id = "content";
                return window.tinyMCEPreInit.mceInit[id] && window.tinyMCEPreInit.mceInit[id].wpautop && window.switchEditors && window.switchEditors.wpautop ? window.switchEditors.pre_wpautop(window.switchEditors.wpautop(string)) : string
            },
        ...
        var Shortcode = Backbone.Model.extend({
            ...
            Shortcodes = Backbone.Collection.extend({
                ...
                create: function(model, options) {
                    return model = Shortcodes.__super__.create.call(this, model, options), model.get("cloned") && vc.events.triggerShortcodeEvents("clone", model), vc.events.triggerShortcodeEvents("add", model), model
                }
            });
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function() {
            ...
            saveTemplate: function(e) {
                ...
                var data, shortcodes, name = this.$name.val(),
                    _this = this;
                return _.isString(name) && name.length ? (shortcodes = this.getPostContent(),
                    shortcodes.trim().length ? (data = {
        ...
        vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
            ...
            show: function() {
                ...
                /*creativo addition*/
                //set counts
                $('.vc_edit-form-tab[data-tab="default_templates"] .template_categ ul > li').each(function(){
                    if($(this).attr('data-sort') == 'all') {
                          $(this).find('.counter').html( $('.vc_edit-form-tab[data-tab="default_templates"] .vc_ui-template-list > .vc_ui-template').length );
                    } else {
                        $(this).find('.counter').html( $('.vc_edit-form-tab[data-tab="default_templates"] .vc_ui-template-list > .vc_ui-template.'+$(this).attr('data-sort')).length );
                    }
                });

                //filtering
                $('.vc_edit-form-tab[data-tab="default_templates"] .template_categ li[data-sort="all"]').addClass('active').trigger('click');

                $('.vc_edit-form-tab[data-tab="default_templates"] .template_categ li').click(function(){
                    $('.vc_edit-form-tab[data-tab="default_templates"] .template_categ li').removeClass('active');
        ...
        vc.map = {}, vc.setFrameSize = function(size) {
            ...
            return paramsMap = vc.getDefaultsAndDependencyMap(tag), outputParams = {}, values = _.extend({}, paramsMap.defaults, values), paramsDependencies = _.extend({}, paramsMap.dependencyMap), _.each(values, function(value, key) {
                if ("content" !== key) {
                    ...
                    if (!_.isUndefined(paramsDependencies[key])) {
                        ...
                        dependedElement = paramsDependencies[key].element, dependedValue = values[dependedElement];
            ...
            return newOrder = _.isBoolean(child_of_clone) && !0 === child_of_clone ? model.get("order") : parseFloat(model.get("order")) + vc.clone_index, params = _.extend({}, model.get("params")), tag = model.get("shortcode"), data = {
                shortcode: tag,
                parent_id: parent_id,
                order: newOrder,
                cloned: !0,
            ...
            var mod = 12 % column;
            return 0 < mod && function(n) {
                return _.isNumber(n) && 1 == n % 2
            }(column) && column % 3 ? column + "/12" : (0 === mod && (mod = column), column / mod + "/" + 12 / mod)
            ...
            update: function(model) {
                this.beforeUpdate(), vc.builder.update(model)
            },
            clone: function(e) {
            ...
            addTextBlock: function(e) {
                ...
                e.preventDefault(), row_params = {}, column_params = {
                    width: "1/1"
                }, column_text_params = vc.getDefaults("vc_column_text"), builder = new vc.ShortcodesBuilder, builder.create({
                    shortcode: "vc_row",
                    params: row_params
                }).create({
                    shortcode: "vc_column",
            ...
            saveElementOrder: function(event, ui) {
                _.defer(function(app, e, ui) {
                    ...
                    _.isNull(ui.sender) && ($column = ui.item.parent(), $elements = $column.find("> [data-model-id]"), $column.find("> [data-model-id]").each(function(key, value) {
                        ...
                        $element = $(this), prepend = !1, $element.is(".droppable") ? (current_parent = vc.shortcodes.get($column.parents(".vc_element[data-tag]:first").data("modelId")), $element.remove(), 0 === key ? prepend = !0 : key + 1 !== $elements.length && (prepend = $column.find("> [data-tag]:eq(" + (key - 1) + ")").data("modelId")), current_parent && vc.add_element_block_view.render(current_parent, prepend)) : (model = vc.shortcodes.get($element.data("modelId")), prev_parent = model.get("parent_id"), current_parent = $column.parents(".vc_element[data-tag]:first").data("modelId"), model.save({
                            order: key,
                            parent_id: current_parent
                        }, {
                            silent: !0
            ...
            saveColumnOrder: function(event, ui) {
                _.defer(function(app, e, ui) {
                    var row;
                    row = ui.item.parent(), row.find("> [data-model-id]").each(function() {
                        var $element, index;
                        $element = $(this), index = $element.index(), vc.shortcodes.get($element.data("modelId")).save({
                            order: index
                        })
                    })
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.addTemplateFilter(function(string) {
        ...
        vc.events.on("shortcodeView:updated", function(model) {
            ...
            settings = vc.map[model.get("shortcode")] || !1, !0 === settings.is_container && (modelId = model.get("id"), vc.frame_window.vc_iframe.updateChildGrids(modelId))
            ...
            addControls: function() {
                ...
                shortcodeTag = this.model.get("shortcode"), template = $(this.controls_selector).html();
                var parentName;
                parent = vc.shortcodes.get(this.model.get("parent_id")), parent && (parentName = vc.getMapped(parent.get("shortcode")).name, parentShortcodeTag = parent.get("shortcode")), allAccess = vc_user_access().shortcodeAll(shortcodeTag), editAccess = vc_user_access().shortcodeEdit(shortcodeTag), parentAllAccess = vc_user_access().shortcodeAll(parentShortcodeTag), parentEditAccess = vc_user_access().shortcodeEdit(parentShortcodeTag), moveAccess = vc_user_access().partAccess("dragndrop"), data = {
                    name: vc.getMapped(shortcodeTag).name,
                    tag: shortcodeTag,
        ...
        window.InlineShortcodeViewContainerWithParent = window.InlineShortcodeViewContainer.extend({
            ...
            switchControls: function(e) {
                ...
                var $current, $control = $(e.currentTarget),
                    $parent = $control.parent();
                $parent.addClass("vc_active"), $current = $parent.siblings(".vc_active"), $current.removeClass("vc_active"), !$current.hasClass("vc_element") && window.setTimeout(this.holdActive, 500)
            }
        })
    }(window.jQuery), function($) {
        ...
        window.InlineShortcodeView_vc_column = window.InlineShortcodeViewContainerWithParent.extend({
            ...
            customCssClassReplace: function() {
                ...
                css_classes = this.$el.find(".wpb_column").attr("class"), css_regex = /.*(vc_custom_\d+).*/, (class_match = !(!css_classes || !css_classes.match) && css_classes.match(css_regex)) && class_match[1] && (this.$el.addClass(class_match[1]), this.$el.find(".wpb_column").attr("class", css_classes.replace(class_match[1], "").trim()))
            },
            ...
            resize: function(e) {
                ...
                diff = e.pageX - this._x, Math.abs(diff) < this._grid_step || (this._x = parseInt(e.pageX), old_width = "" + this.css_class_width, 0 < diff ? this.css_class_width += 1 : 0 > diff && (this.css_class_width -= 1), 12 < this.css_class_width && (this.css_class_width = 12), 1 > this.css_class_width && (this.css_class_width = 1), params.width = vc.getColumnSize(this.css_class_width), this.model.save({
                    params: params
                }, {
                    silent: !0
        ...
        window.InlineShortcodeView_vc_tta_accordion = window.InlineShortcodeViewContainer.extend({
            ...
            addSection: function(prepend) {
                ...
                for (shortcode = this.childTag, params = {
                    ...
                    }, prepend && (vc.activity = "prepend", params.order = this.getSiblingsFirstPositionIndex()), vc.builder.create(params), i = vc.builder.models.length - 1; i >= 0; i--) shortcode = vc.builder.models[i].get("shortcode");
                vc.builder.render()
            },
            getSiblingsFirstPositionIndex: function() {
                ...
                return order = 0, shortcodeFirst = vc.shortcodes.sort().findWhere({
                    parent_id: this.model.get("id")
                }), shortcodeFirst && (order = shortcodeFirst.get("order") - 1), order
            },
            changed: function() {
            ...
            updateSorting: function(event, ui) {
                ...
                this.getPanelsList().find("> .vc_element").each(function() {
                    ...
                    $this = $(this), modelId = $this.data("modelId"), shortcode = vc.shortcodes.get(modelId), shortcode.save({
            ...
            getPaginationList: function() {
                ...
                return params = this.model.get("params"), !_.isUndefined(params.pagination_style) && params.pagination_style.length ? ($accordions = this.$el.find("[data-vc-accordion]"), classes = [], classes.push("vc_general"), classes.push("vc_pagination"), styleChunks = params.pagination_style.split("-"), classes.push("vc_pagination-style-" + styleChunks[0]), classes.push("vc_pagination-shape-" + styleChunks[1]), !_.isUndefined(params.pagination_color) && params.pagination_color.length && classes.push("vc_pagination-color-" + params.pagination_color), html = [],
                    html.push('<ul class="' + classes.join(" ") + '">'), that = this, $accordions.each(function() {
                        var sectionClasses, activeSection, $this, $closestPanel, selector, aHtml;
                        $this = $(this), $closestPanel = $this.closest(".vc_tta-panel"), activeSection = $closestPanel.hasClass(that.activeClass), sectionClasses = ["vc_pagination-item"], activeSection && sectionClasses.push(that.activeClass), selector = $this.attr("href"), 0 !== selector.indexOf("#") && (selector = ""), $this.attr("data-vc-target") && (selector = $this.attr("data-vc-target")), aHtml = '<a href="javascript:;" data-vc-target="' + selector + '" class="vc_pagination-trigger" data-vc-tabs data-vc-container=".vc_tta"></a>', html.push('<li class="' + sectionClasses.join(" ") + '" data-vc-tab>' + aHtml + "</li>")
        ...
        window.InlineShortcodeView_vc_tta_tabs = window.InlineShortcodeView_vc_tta_accordion.extend({
            ...
            addIcon: function(model, html) {
                ...
                return "true" === model.getParam("add_icon") && (icon = model.getParam("i_icon_" + model.getParam("i_type")), _.isUndefined(icon) || (iconClass = "vc_tta-icon " + icon, iconHtml = '<i class="' + iconClass + '"></i>'), "right" === model.getParam("i_position") ? html += iconHtml : html = iconHtml + html), html
            },
            sectionUpdated: function(model, justAppend) {
                var $tabEl, $navigation, sectionId, html, title, models, index, tabAdded;
                if (tabAdded = !1, sectionId = model.get("id"), $navigation = this.$el.find(".vc_tta-tabs-container .vc_tta-tabs-list"), $tabEl = $navigation.find('[data-vc-target="[data-model-id=' + sectionId + ']"]'), title = model.getParam("title"), $tabEl.length) html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $tabEl.html(html);
                else {
                    var $element;
                    html = '<span class="vc_tta-title-text">' + title + "</span>", html = this.addIcon(model, html), $element = $('<li class="vc_tta-tab" data-vc-target-model-id="' + sectionId + '" data-vc-tab><a href="javascript:;" data-vc-use-cache="false" data-vc-tabs data-vc-target="[data-model-id=' + sectionId + ']" data-vc-container=".vc_tta">' + html + "</a></li>"), !0 !== justAppend && (models = _.pluck(_.sortBy(vc.shortcodes.where({
                        parent_id: this.model.get("id")
                    }), function(childModel) {
                        return childModel.get("order")
                    }), "id"), (index = models.indexOf(model.get("id")) - 1) > -1 && $navigation.find("[data-vc-tab]:eq(" + index + ")").length && ($element.insertAfter($navigation.find("[data-vc-tab]:eq(" + index + ")")), tabAdded = !0)), !tabAdded && $element.appendTo($navigation), model.get("isActiveSection") && $element.addClass(this.activeClass)
                }
                this.buildPagination()
            },
            getNextTab: function($viewTab) {
                var lastIndex, viewTabIndex, $navigationSections;
                return $navigationSections = this.$el.find(".vc_tta-tabs-container .vc_tta-tabs-list").children(), lastIndex = $navigationSections.length - 1, viewTabIndex = $viewTab.index(), viewTabIndex !== lastIndex ? $navigationSections.eq(viewTabIndex + 1) : $navigationSections.eq(viewTabIndex - 1)
            },
            removeSection: function(modelId) {
                var $viewTab, $nextTab, tabIsActive;
                $viewTab = this.$el.find('.vc_tta-tabs-container .vc_tta-tabs-list [data-vc-target="[data-model-id=' + modelId + ']"]').parent(), tabIsActive = $viewTab.hasClass(this.activeClass), tabIsActive && ($nextTab = this.getNextTab($viewTab), vc.frame_window.jQuery($nextTab).find("[data-vc-target]").trigger("click")), $viewTab.remove(), this.buildPagination()
            },
            ...
            updateSortingNavigation: function() {
                var $tabs, self;
                self = this, $tabs = this.$el.find(".vc_tta-tabs-list"), $tabs.find("> .vc_tta-tab").each(function() {
                    var shortcode, modelId, $li;
                    $li = $(this).removeAttr("style"), modelId = $li.data("vcTargetModelId"), shortcode = vc.shortcodes.get(modelId), shortcode.save({
            ...
            updateTabsPositions: function($panels) {
                ...
                $tabs = this.$el.find(".vc_tta-tabs-list"), $tabs.length && ($elements = [], tabSortableData = $panels.sortable("toArray", {
        ...
        vc.cloneMethod_vc_tab = function(data, model) {
            ...
            buildControlHtml: function(model) {
                var $tab = (model.get("params"), $('<li data-m-id="' + model.get("id") + '"><a href="#tab-' + model.getParam("tab_id") + '"></a></li>'));
        ...
        window.InlineShortcodeView_vc_accordion_tab = window.InlineShortcodeView_vc_tab.extend({
            ...
            window.InlineShortcodeView_recent_works = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_recent_works.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.recent_works(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_vc_clients = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_vc_clients.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                            this.vc_iframe.vc_clients(model_id)
                        }),
                        this
                }
            }), window.InlineShortcodeView_woo_carousel = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_woo_carousel.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.woo_carousel(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_gmap_advanced = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_gmap_advanced.__super__.render.call(this),
                        vc.frame_window.vc_iframe.addActivity(function() {
                            this.vc_iframe.gmap_advanced(model_id)
                        }), this
                }
            }), window.InlineShortcodeView_vc_testimonials = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_vc_testimonials.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.vc_testimonials(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_vc_carousel2 = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id =
                        this.model.get("id");
                    return window.InlineShortcodeView_vc_carousel2.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.vc_carousel2(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_cr_posts_slider = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id =
                        this.model.get("id");
                    return window.InlineShortcodeView_cr_posts_slider.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.cr_posts_slider(model_id)
                    }), this
                }    
            }), window.InlineShortcodeView_vc_counter = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_vc_counter.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.vc_counter(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_countdown_el =
            window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_countdown_el.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.countdown_el(model_id)
                    }), this
                }
            }), window.InlineShortcodeView_recent_posts = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_recent_posts.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                            this.vc_iframe.recent_posts(model_id)
                        }),
                        this
                }
            }), window.InlineShortcodeView_flip_box = window.InlineShortcodeView.extend({
                render: function() {
                    var model_id = this.model.get("id");
                    return window.InlineShortcodeView_flip_box.__super__.render.call(this), vc.frame_window.vc_iframe.addActivity(function() {
                        this.vc_iframe.flip_box(model_id)
                    }), this
                }
            })
    }(window.jQuery), _.isUndefined(vc)) var vc = {};
! function($) {
    "use strict";
    ...
    }, vc.createPreLoader(), vc.$frame_wrapper = $("#vc_inline-frame-wrapper"), vc.$frame = $('<iframe src="' + vc_iframe_src + '" scrolling="auto" style="width: 100%;" id="vc_inline-frame"></iframe>'), vc.$frame.appendTo(vc.$frame_wrapper), vc.build = function() {
        if (!vc.loaded) {
            vc.loaded = !0, vc.map = window.vc_mapper, $("#wpadminbar").remove(), $("#screen-meta-links, #screen-meta").hide();
            var $body = $("body");
            $body.attr("data-vc", !0), vc.post_id = $("#vc_post-id").val(), vc.is_mobile = 0 < $("body.mobile").length, vc.title = $("#vc_title-saved").val(), vc.add_element_block_view = new vc.AddElementUIPanelFrontendEditor({
                el: "#vc_ui-panel-add-element"
            ...
            }), vc.app.render(), vc.post_shortcodes = vc.frame_window.vc_post_shortcodes, vc.builder.buildFromContent(), vc.removePreLoader(), vc.$frame.get(0).contentWindow.vc_js && vc.$frame.get(0).contentWindow.vc_js(), $(window).trigger("vc_build")
        }
