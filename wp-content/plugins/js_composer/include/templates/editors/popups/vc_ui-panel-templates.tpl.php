			...
			<div class="vc_ui-panel-content vc_properties-list"
				...
				<div class="vc_panel-tabs">
					...
					foreach ( $categories_data as $key => $category ) :
						...
						/*creativo additon - library sidebar*/
						if(esc_attr( $category['category'] ) == 'default_templates') {
							echo '<div class="template_categ">
								<ul>';
									$creativo_library_cats = creativo_vc_library_cat_list();
									foreach($creativo_library_cats as $cat_name => $cat_sort_text) {
										echo '<li data-sort="'.$cat_sort_text.'">'.$cat_name.' <span class="counter">0</span></li>';
									}
								echo '</ul>
							</div>';
						}
						/* creativo additon end */
