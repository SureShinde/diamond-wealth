...
if($color == 'custom' || $color == 'custom_grd') {
	$styles_render = '<style type="text/css" scoped="scoped">';
	
		$styles_render .= '.button.button_scope_' . $button_counter . ',.button.button_scope_' . $button_counter . '.style_3d:hover{';			
			$styles_render .= 'color:'.$custom_text_color.';';
			$styles_render .= 'padding-left: ' . $padding_lr . '; padding-right: '.$padding_lr.';';
			if($custom_font_size != '') {
				$styles_render .= 'font-size:'.$custom_font_size.';';
			}
			if($custom_line_height !=''){
				$styles_render .= 'line-height:'.$custom_line_height.';';
			}
			if($custom_font_weight !=''){
				$styles_render .= 'font-weight:'.$custom_font_weight.';';
			}
			if($custom_letter_spacing !='1px') {
				$styles_render .= 'letter-spacing:'.$custom_letter_spacing.';';
			}
			if($color == 'custom') {
				$styles_render .= 'border-color:'.$custom_border_color.';';
				$styles_render .= 'background-color:'.$custom_color.';';
				$styles_render .= 'border-width:'.$button_border_width.';';
			}
			else {
				$styles_render .= 'border: none;';
				$styles_render .= 'background:'.$grd_color_1.';';				
				$styles_render .= 'background: -moz-linear-gradient(top, '.$grd_color_1.' 0%, '.$grd_color_2.' 100%);';
				$styles_render .= 'background: -webkit-linear-gradient(top, '.$grd_color_1.' 0%, '.$grd_color_2.' 100%);';
				$styles_render .= 'background: linear-gradient(top, '.$grd_color_1.' 0%, '.$grd_color_2.' 100%);';
				$styles_render .= 'filter: progid:DXImageTransform.Microsoft.gradient( startColorstr=\''.$grd_color_1.'\', endColorstr=\''.$grd_color_2.'\',GradientType=0 )';
			}
		$styles_render .= '}';
		
		if($color == 'custom') {
			$styles_render .= '.button.button_scope_' . $button_counter . ':hover{';
			...
			$styles_render .= '.button.button_scope_' . $button_counter . '.style_3d {';
			...
			$styles_render .= '.button.button_scope_' . $button_counter . '.style_3d {';
				$styles_render .= 'box-shadow: 0 5px 0' . hexDarker( $grd_color_2, 20 ).';';
				$styles_render .= '-webkit-box-shadow: 0 5px 0' . hexDarker( $grd_color_2, 20 ).';';
			$styles_render .= '}';
		}
