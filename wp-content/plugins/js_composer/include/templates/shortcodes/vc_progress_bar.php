 ...
 */
$title = $values = $units = $bgcolor = $css = $custombgcolor = $custom_bar_height = $customtxtcolor = $options = $el_class = $el_id = $css_animation = '';
$output = '';
...
$custom_bar_height = ( $custom_bar_height !='7px') ? 'style="height:'.$custom_bar_height.';"' : '';
...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ? ' <span class="vc_label_units" >' . $line['value'] . $units . '</span>' : '';
		$output .= '<div class="progress_bar_description" ' . $line['txtcolor'] . '>' . $line['label'] . $unit . '</div>';
		$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
				' vc_progress-bar-color-' . $line['color'] : '' )
			. '">';
		$output .= '<small class="vc_label"' . $line['txtcolor'] . ' '.$custom_bar_height .'></small>';
		if ( $max_value > 100.00 ) {
